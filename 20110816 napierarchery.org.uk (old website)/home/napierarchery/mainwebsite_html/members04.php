...
<? $memberCount =& $db->getAll("SELECT * FROM members04"); ?>
<div style="width:100%">
<h3>Members 2004-2005</h3>
<center>
...
function displayMember($memberID) {
  global $db;
  $member =& $db->getAll("SELECT * FROM members04 WHERE members04.memberid = ".$memberID);
  $quotes =& $db->getAll("SELECT quote FROM quotes04 WHERE quotes04.memberid = ".$memberID);
...
function doChoice() {
  echo '<div style="width:100%">';
  echo '<center>';
  echo '<form action="members04.php" method="post">';
    
  global $db;
  
  $members =& $db->getAll("SELECT * FROM members04 ORDER BY name ASC");
