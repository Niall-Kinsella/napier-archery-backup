...
<? $memberCount =& $db->getAll("SELECT * FROM members05"); ?>
<div style="width:100%">
...
function displayMember($memberID) {
  global $db;
  $member =& $db->getAll("SELECT * FROM members05 WHERE members05.memberid = ".$memberID);
  $quotes =& $db->getAll("SELECT quote FROM quotes05 WHERE quotes05.memberid = ".$memberID);
...
function doChoice() {
  echo '<div style="width:100%">';
  echo '<center>';
  echo '<form action="members05.php" method="post">';
    
  global $db;
  
  $members =& $db->getAll("SELECT * FROM members05 ORDER BY name ASC");
